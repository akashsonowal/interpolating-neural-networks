...
class MLPDistributedTrainer:
  ...
  def train_step(self, inputs, model, global_batch_size):
  ...
  def val_step(self, inputs, model):
    features, labels = inputs
  ...
  @tf.function
  def distributed_train_step(dataset_inputs, model, global_batch_size):
    per_replica_losses = self.strategy.run(self.train_step, args=(dataset_inputs, model, global_batch_size))
    return strategy.reduce(tf.distribute.ReduceOp.SUM, per_replica_losses,
                           axis=None)

  @tf.function
  def distributed_val_step(dataset_inputs, model, global_batch_size):
    return self.strategy.run(self.val_step, args=(dataset_inputs, model, global_batch_size))
  ...
  def fit(self, model, train_dataloader, val_dataloader, global_batch_size):
      for epoch in range(self.epochs):
        ...
        for x in train_dataloader:
          total_loss += self.distributed_train_step(x, model, global_batch_size)
          num_batches += 1
        train_loss = total_loss / num_batches
        # Validation Loop
        for x in val_dataloader:
          self.distributed_val_step(x, model)
